...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    posKeyTT = excludedMove == MOVE_NONE ? pos.keyTT() : pos.keyTT() ^ make_key(excludedMove);
    tte = TT.probe(posKeyTT, ss->ttHit, us);
    ttValue = ss->ttHit ? value_from_tt(tte->value(us), ss->ply, pos.rule50_count()) : VALUE_NONE;
    ttMove =  rootNode ? thisThread->rootMoves[thisThread->pvIdx].pv[0]
            : ss->ttHit    ? tte->move(us) : MOVE_NONE;
    ttCapture = ttMove && pos.capture_or_promotion(ttMove);
    if (!excludedMove)
        ss->ttPv = PvNode || (ss->ttHit && tte->is_pv(us));

    // At non-PV nodes we check for an early TT cutoff
    if (  !PvNode
        && ss->ttHit
        && tte->depth(us) > depth - (thisThread->id() % 2 == 1)
        && ttValue != VALUE_NONE // Possible in case of TT access race
        && (ttValue >= beta ? (tte->bound(us) & BOUND_LOWER)
                            : (tte->bound(us) & BOUND_UPPER)))
    {
    ...
    if (!rootNode && TB::Cardinality)
    {
            ...
            && !pos.can_castle(ANY_CASTLING))
        {
            ...
            if (err != TB::ProbeState::FAIL)
            {
                ...
                {
                    tte->save(posKeyTT, value_to_tt(value, ss->ply), ss->ttPv, b,
                              std::min(MAX_PLY - 1, depth + 6),
                              MOVE_NONE, VALUE_NONE, us);
    ...
    else if (ss->ttHit)
    {
        ...
        ss->staticEval = eval = tte->eval(us);
        if (eval == VALUE_NONE)
        ...
        if (    ttValue != VALUE_NONE
            && (tte->bound(us) & (ttValue > eval ? BOUND_LOWER : BOUND_UPPER)))
            eval = ttValue;
    }
    else
    {
        ...
        if (!excludedMove)
            tte->save(posKeyTT, VALUE_NONE, ss->ttPv, BOUND_NONE, DEPTH_NONE, MOVE_NONE, eval, us);
    }
            ...
            if (move != excludedMove && pos.legal(move))
            {
                ...
                if (value >= probCutBeta)
                {
                    ...
                    if ( !(ss->ttHit
                       && tte->depth(us) >= depth - 3
                       && ttValue != VALUE_NONE))
                        tte->save(posKeyTT, value_to_tt(value, ss->ply), ttPv,
                            BOUND_LOWER,
                            depth - 3, move, ss->staticEval, us);
                    return value;
                }
            }
         ss->ttPv = ttPv;
...
moves_loop: // When in check, search starts here
    ...
    */
    ...
    if (!excludedMove && !(rootNode && thisThread->pvIdx))
        tte->save(posKeyTT, value_to_tt(bestValue, ss->ply), ss->ttPv,
                  bestValue >= beta ? BOUND_LOWER :
                  PvNode && bestMove ? BOUND_EXACT : BOUND_UPPER,
                  depth, bestMove, ss->staticEval, us);
  ...
  Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    tte = TT.probe(posKeyTT, ss->ttHit, us);
    ttValue = ss->ttHit ? value_from_tt(tte->value(us), ss->ply, pos.rule50_count()) : VALUE_NONE;
    ttMove = ss->ttHit ? tte->move(us) : MOVE_NONE;
    pvHit = ss->ttHit && tte->is_pv(us);

    if (  !PvNode
        && ss->ttHit
        && tte->depth(us) >= ttDepth
    ...
    else
    {
        ...
        {
            // Never assume anything about values stored in TT
            if ((ss->staticEval = bestValue = tte->eval(us)) == VALUE_NONE)
                ss->staticEval = bestValue = evaluate(pos);

            // ttValue can be used as a better position evaluation (~7 Elo)
            if (    ttValue != VALUE_NONE
                && (tte->bound(us) & (ttValue > bestValue ? BOUND_LOWER : BOUND_UPPER)))
                bestValue = ttValue;
        }
        else
        ...
        if (bestValue >= beta)
        {
            ...
            if (!ss->ttHit)
                tte->save(posKeyTT, value_to_tt(bestValue, ss->ply), false, BOUND_LOWER,
                          DEPTH_NONE, MOVE_NONE, ss->staticEval, us);
    ...
    // Save gathered info in transposition table
    tte->save(posKeyTT, value_to_tt(bestValue, ss->ply), pvHit,
              bestValue >= beta ? BOUND_LOWER : BOUND_UPPER,
              ttDepth, bestMove, ss->staticEval, us);
...
bool RootMove::extract_ponder_from_tt(Position& pos) {
    ...
    pos.do_move(pv[0], st);
    TTEntry* tte = TT.probe(pos.key(), ttHit, pos.side_to_move());

    if (ttHit)
    {
        Move m = tte->move(pos.side_to_move()); // Local copy to be SMP safe
